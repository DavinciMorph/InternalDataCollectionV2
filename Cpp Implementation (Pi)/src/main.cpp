...
int main(int argc, char* argv[]) {
    ...
    if (args.check_impedance) {
        ...
        for (auto* dev : active_devices) {
            ...
            // SRB1 ON — provides excitation current return path
            dev->write_register(ads1299::Reg::MISC1, 0x20);
            sleep_sec(0.01);
            // LOFF: 6µA AC excitation
            dev->write_register(ads1299::Reg::LOFF, 0x08);
            sleep_sec(0.01);
            // P-side excitation only (current source on each P electrode)
            // N-side off — return path is through SRB1, not individual N electrodes
