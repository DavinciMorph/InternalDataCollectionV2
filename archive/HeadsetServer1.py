...
class ADS1299_Controller:
    ...
    def start_all_conversions_synchronized(spi_devices: List['ADS1299_SPI']) -> bool:
        ...
        # Final re-synchronization: multiple stop/start cycles to ensure perfect alignment
        # The key insight is that Port1 Device 3 (last in short chain) is most sensitive
        # Multiple cycles give devices repeated opportunities to align properly
        print(f"\n  Final re-synchronization (multiple cycles)...")

        start_controller = ADS1299_Controller._create_start_controller(spi_devices)
        DIAG_THRESHOLD = 6000
        MAX_RESYNC_CYCLES = 5
        SAMPLES_PER_CHECK = 200
        resync_clean = False  # Track if we achieved a clean resync

        for cycle in range(MAX_RESYNC_CYCLES):
            print(f"\n    Re-sync cycle {cycle + 1}/{MAX_RESYNC_CYCLES}...")

            # Stop all conversions
            start_controller.set_all_low()
            time.sleep(0.100)

            # Send STOP and SDATAC to ensure clean state
            ADS1299_Controller._broadcast_command(spi_devices, ADS1299_Cmd.STOP, delay_ms=10)
            time.sleep(0.100)
            ADS1299_Controller._broadcast_command(spi_devices, ADS1299_Cmd.SDATAC, delay_ms=20)
            time.sleep(0.100)

            # Re-enter RDATAC mode - send multiple times for reliability
            for _ in range(3):
                for spi_dev in spi_devices:
                    spi_dev.send_command(ADS1299_Cmd.RDATAC)
                time.sleep(0.050)

            # Longer settling after RDATAC
            time.sleep(0.500)
            ...
            # Check for corruption in this cycle
            ref_port = spi_devices[2] if len(spi_devices) > 2 else spi_devices[0]
            cycle_corruption = 0
            device_hits = {}  # Track which devices have corruption

            for sample_idx in range(SAMPLES_PER_CHECK):
                        ...
                        for device_idx, device_channels in enumerate(channel_data):
                            for value in device_channels:
                                if value > DIAG_THRESHOLD or value < -DIAG_THRESHOLD:
                                    cycle_corruption += 1
                                    key = f"{port_id} Dev{device_idx + 1}"
                                    device_hits[key] = device_hits.get(key, 0) + 1

            if cycle_corruption == 0:
                print(f"    Cycle {cycle + 1}: CLEAN ({SAMPLES_PER_CHECK} samples, 0 corruption) ✓")
                print(f"  ✓ Re-synchronization successful on cycle {cycle + 1}")
                resync_clean = True
                break
            else:
                hit_str = ", ".join([f"{k}:{v}" for k, v in sorted(device_hits.items())])
                print(f"    Cycle {cycle + 1}: {cycle_corruption} corruptions ({hit_str})")
                if cycle < MAX_RESYNC_CYCLES - 1:
                    print(f"    Retrying...")
        else:
            print(f"  ⚠ Re-synchronization: No clean cycle after {MAX_RESYNC_CYCLES} attempts")

        # Measure per-port DRDY timing and SPI read times
        ref_port = spi_devices[2] if len(spi_devices) > 2 else spi_devices[0]
        port_read_times = {spi_dev.config.port_id: [] for spi_dev in spi_devices}

        print(f"\n  Measuring per-port timing (10 samples)...")
        for sample_idx in range(10):
            if ref_port.wait_for_drdy(timeout=0.02):
                for spi_dev in spi_devices:
                    ...
                    port_read_times[spi_dev.config.port_id].append(read_elapsed)

        # Measure DRDY timing for each port individually
        print(f"\n  Per-port DRDY timing:")
        per_port_drdy = {}
        for spi_dev in spi_devices:
            port_id = spi_dev.config.port_id
            drdy_times = []
            for _ in range(10):
                ...
                if spi_dev.wait_for_drdy(timeout=0.02):
                    drdy_times.append((time.time() - drdy_start) * 1000)
                    spi_dev.read_data()
            if drdy_times:
                per_port_drdy[port_id] = sum(drdy_times) / len(drdy_times)
                print(f"    {port_id}: avg DRDY wait {per_port_drdy[port_id]:.2f}ms")

        # Check for DRDY timing spread between ports
        if per_port_drdy:
            min_drdy = min(per_port_drdy.values())
            max_drdy = max(per_port_drdy.values())
            spread = max_drdy - min_drdy
            if spread > 0.5:
                print(f"  ⚠ DRDY timing spread: {spread:.2f}ms (may indicate sync issues)")

        # Report per-port read times
        print(f"\n  Per-port SPI read times:")
        for port_id, times in port_read_times.items():
            if times:
                avg_time = sum(times) / len(times)
                max_time = max(times)
                print(f"    {port_id}: avg {avg_time:.2f}ms, max {max_time:.2f}ms")

        # Extended diagnostic: run 500 more samples to try to catch late-emerging corruption
        print(f"\n  Extended diagnostics (500 samples to detect late-emerging issues)...")
        extended_corruption = {spi_dev.config.port_id: {} for spi_dev in spi_devices}  # port -> {device_idx: count}
        extended_total = 0
        ...
        for sample_idx in range(500):
            if ref_port.wait_for_drdy(timeout=0.02):
                for spi_dev in spi_devices:
                    ...
                    # Check all channels for corruption - track by device
                    for device_idx, device_channels in enumerate(channel_data):
                        for ch_idx, value in enumerate(device_channels):
                            if value > DIAG_THRESHOLD or value < -DIAG_THRESHOLD:
                                extended_total += 1
                                dev_key = device_idx + 1
                                if dev_key not in extended_corruption[port_id]:
                                    extended_corruption[port_id][dev_key] = 0
                                extended_corruption[port_id][dev_key] += 1

                                # Print first occurrence per device
                                if extended_corruption[port_id][dev_key] == 1:
                                    print(f"    FIRST corruption: {port_id} Device {dev_key} at extended sample {sample_idx}, value={value}")

        if extended_total > 0:
            print(f"  ⚠ EXTENDED DIAGNOSTIC CORRUPTION: {extended_total} total in 500 samples")
            for port_id, devices in extended_corruption.items():
                for dev_idx, count in devices.items():
                    if count > 0:
                        print(f"    {port_id} Device {dev_idx}: {count} corrupted values")
        else:
            print(f"  ✓ Extended diagnostic check: 0 corruption in 500 samples")

        # Register verification - check if CONFIG registers are still correct
        print(f"\n  Verifying registers after warmup...")
        register_issues = []

        # Need to stop conversions briefly to read registers
        start_controller = ADS1299_Controller._create_start_controller(spi_devices)
        start_controller.set_all_low()
        time.sleep(0.050)
        ADS1299_Controller._broadcast_command(spi_devices, ADS1299_Cmd.SDATAC, delay_ms=20)
        ...
        for spi_dev in spi_devices:
            port_id = spi_dev.config.port_id
            # Read CONFIG1, CONFIG2, CONFIG3
            regs = spi_dev.read_registers(ADS1299_Reg.CONFIG1, 3)
            config1, config2, config3 = regs[0], regs[1], regs[2]

            issues = []
            if config1 != 0x96:
                issues.append(f"CONFIG1=0x{config1:02X}(expected 0x96)")
            if config2 != 0xD0:
                issues.append(f"CONFIG2=0x{config2:02X}(expected 0xD0)")
            if config3 != 0xE0:
                issues.append(f"CONFIG3=0x{config3:02X}(expected 0xE0)")

            if issues:
                register_issues.append((port_id, issues))
                print(f"    ⚠ {port_id}: {', '.join(issues)}")
            else:
                print(f"    ✓ {port_id}: Registers verified")

        # Re-enter RDATAC and restart (multiple times for reliability)
        for _ in range(3):
            for spi_dev in spi_devices:
                spi_dev.send_command(ADS1299_Cmd.RDATAC)
            time.sleep(0.050)
        time.sleep(0.200)
        start_controller.set_all_high()
        time.sleep(0.500)  # Match resync settling time

        # Discard 50 samples after register check pause
        for _ in range(50):
            if ref_port.wait_for_drdy(timeout=0.02):
                for spi_dev in spi_devices:
                    spi_dev.read_data()
        ...
        # Final verdict
        if not resync_clean or extended_total > 0 or register_issues:
            issues_str = []
            if not resync_clean:
                issues_str.append("no clean resync cycle")
            if extended_total > 0:
                issues_str.append(f"{extended_total} corrupted in extended check")
            if register_issues:
                issues_str.append(f"{len(register_issues)} ports with register drift")
            print(f"\n  *** WARNING: Init issues ({', '.join(issues_str)}) - run may have problems ***")
        else:
            print(f"\n  *** CLEAN INIT: Resync clean, extended check clean, registers verified ***")
