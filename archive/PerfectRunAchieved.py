...
class ADS1299_Controller:
    ...
    @staticmethod
    def initialize_device(spi: ADS1299_SPI, config: ADS1299_Config, max_retries: int = 5) -> bool:
        """Initialize and configure a single ADS1299 bus using software reset. Returns True if successful."""
        print(f"\nConfiguring {spi.config.port_id}...")

        for attempt in range(max_retries):
            if attempt > 0:
                print(f"  Retry {attempt}/{max_retries-1} for {spi.config.port_id}...")
            ...
            # Software reset
            if attempt == 0:
                print(f"  Performing software reset...")
            spi.send_command(ADS1299_Cmd.RESET)
            time.sleep(0.5)
            if attempt == 0:
                print(f"  Software reset complete ✓")
            ...
            if id_value == 0x00:
                print(f"  ✗ ERROR: ID=0x00 - no device, SPI disabled, no clock, or power issue")
                return False
            ...
            spi.write_registers(ADS1299_Reg.CONFIG1, [config.config1])
            time.sleep(0.01)
            spi.write_registers(ADS1299_Reg.CONFIG2, [config.config2])
            time.sleep(0.01)
            spi.write_registers(ADS1299_Reg.CONFIG3, [config.config3])
            time.sleep(0.15)  # Reference buffer settling

            # Re-write CONFIG2/3 to ensure they stick
            spi.write_registers(ADS1299_Reg.CONFIG2, [config.config2])
            spi.write_registers(ADS1299_Reg.CONFIG3, [config.config3])
            time.sleep(0.01)

            # Write channel settings with retry
            channel_settings = config.get_channel_settings()
            ch_settings_ok = False

            for ch_attempt in range(10):
                for i, ch_val in enumerate(channel_settings):
                    spi.write_registers(ADS1299_Reg.CH1SET + i, [ch_val])
                    time.sleep(0.003)
                time.sleep(0.030)

                ch_readback = spi.read_registers(ADS1299_Reg.CH1SET, 8)
                ch_mismatch = False
                mismatched_channels = []

                for i, (expected, actual) in enumerate(zip(channel_settings, ch_readback)):
                    if expected != actual:
                        ch_mismatch = True
                        mismatched_channels.append((i+1, actual, expected))

                if ch_mismatch:
                    if ch_attempt == 0 or ch_attempt >= 7:
                        for ch_num, actual, expected in mismatched_channels:
                            print(f"  CH{ch_num}SET: 0x{actual:02X} (expected 0x{expected:02X}) ✗")
                    if ch_attempt < 9:
                        for ch_num, actual, expected in mismatched_channels:
                            for _ in range(3):
                                spi.write_registers(ADS1299_Reg.CH1SET + (ch_num - 1), [expected])
                                time.sleep(0.005)
                        time.sleep(0.050)
                        continue
                    else:
                        print(f"  ⚠ Channel settings failed after 10 attempts - continuing")
                else:
                    ch_settings_ok = True
                    if ch_attempt > 0:
                        print(f"  Channel settings verified on attempt {ch_attempt + 1} ✓")
                    break

            if not ch_settings_ok and attempt == 0:
                print(f"  ⚠ Some channel settings may not have been applied correctly")
            ...
            # Verify CONFIG1-3
            readback = spi.read_registers(ADS1299_Reg.CONFIG1, 3)

            config1_ok = readback[0] == config.config1
            config2_ok = readback[1] == config.config2
            config3_ok = readback[2] == config.config3

            if attempt == 0 or not (config1_ok and config2_ok and config3_ok):
                c1 = '✓' if config1_ok else '✗'
                c2 = '✓' if config2_ok else '✗'
                c3 = '✓' if config3_ok else '✗'
                print(f"  CONFIG1=0x{readback[0]:02X}{c1} CONFIG2=0x{readback[1]:02X}{c2} CONFIG3=0x{readback[2]:02X}{c3}")

            if config1_ok and config2_ok and config3_ok:
                print(f"  ✓ {spi.config.port_id} configured successfully")
                return True

            if config1_ok and not config2_ok and config3_ok:
                spi.write_registers(ADS1299_Reg.CONFIG2, [config.config2])
                time.sleep(0.05)
                readback2 = spi.read_registers(ADS1299_Reg.CONFIG2, 1)[0]
                if readback2 == config.config2:
                    print(f"  CONFIG2: 0x{readback2:02X} (expected 0x{config.config2:02X}) ✓ (fixed on re-write)")
                    print(f"  ✓ {spi.config.port_id} configured successfully")
                    return True

            if config1_ok and config2_ok and not config3_ok:
                spi.write_registers(ADS1299_Reg.CONFIG3, [config.config3])
                time.sleep(0.5)
                readback3 = spi.read_registers(ADS1299_Reg.CONFIG3, 1)[0]
                if readback3 == config.config3:
                    print(f"  CONFIG3: 0x{readback3:02X} (expected 0x{config.config3:02X}) ✓ (fixed on re-write)")
                    print(f"  ✓ {spi.config.port_id} configured successfully")
                    return True

            if config1_ok and not config2_ok and not config3_ok:
                spi.write_registers(ADS1299_Reg.CONFIG2, [config.config2])
                time.sleep(0.05)
                spi.write_registers(ADS1299_Reg.CONFIG3, [config.config3])
                time.sleep(0.5)
                readback = spi.read_registers(ADS1299_Reg.CONFIG2, 2)
                if readback[0] == config.config2 and readback[1] == config.config3:
                    print(f"  CONFIG2: 0x{readback[0]:02X} (expected 0x{config.config2:02X}) ✓ (fixed on re-write)")
                    print(f"  CONFIG3: 0x{readback[1]:02X} (expected 0x{config.config3:02X}) ✓ (fixed on re-write)")
                    print(f"  ✓ {spi.config.port_id} configured successfully")
                    return True

        print(f"  ✗ {spi.config.port_id} configuration failed after {max_retries} attempts")
        return False
    ...
    def start_all_conversions_synchronized(spi_devices: List['ADS1299_SPI']) -> bool:
        ...
        time.sleep(0.150)  # 150ms settling
        ...
        time.sleep(0.100)  # 100ms synchronization
        ...
        WARMUP_CORRUPTION_THRESHOLD = 0.50
        bad_start_detected = False
        for port_id, count in warmup_corruptions.items():
            if warmup_count > 0:
                corruption_rate = count / warmup_count
                if corruption_rate > WARMUP_CORRUPTION_THRESHOLD:
                    print(f"  ✗ {port_id} has {corruption_rate*100:.0f}% warmup corruption (threshold: {WARMUP_CORRUPTION_THRESHOLD*100:.0f}%)")
                    bad_start_detected = True

        if bad_start_detected:
            print(f"\n  ✗ High warmup corruption - requesting restart...")
            ADS1299_Controller._set_all_start_pins_low(spi_devices)
...
"""
    ...
    # Each device handles its own retries (up to 5 per device)
    # System-level retry only needed for catastrophic failures
    max_system_attempts = 5
    initialization_successful = False
    
    for sys_attempt in range(max_system_attempts):
        if sys_attempt > 0:
            print(f"\n{'='*60}\nSYSTEM RETRY {sys_attempt + 1}/{max_system_attempts}\n{'='*60}")
            # Full system reset before retry
            for spi_dev in spi_devices:
                spi_dev.start.set_low()
                spi_dev.send_command(ADS1299_Cmd.STOP)
            time.sleep(0.5)  # 500ms for SPI bus to fully settle (increased from 100ms)
            ADS1299_Controller.force_all_start_pins_low(bus_configs)
            ADS1299_Controller.wait_for_power_on()
        ...
        # Configure each device (each has internal retry loop)
        for spi_dev in spi_devices:
            ...
            if not success:
                ...
                time.sleep(0.050)  # 50ms settle before configuring next port
    ...
    if not initialization_successful:
        print(f"\n{'='*60}\n⚠ Data flow incomplete after {max_system_attempts} attempts\n{'='*60}")
