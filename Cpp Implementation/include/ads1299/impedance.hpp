...
//
// Bits [1:0]: FLEAD_OFF — excitation frequency
//   00 = DC lead-off detection
//   01 = AC at 7.8125 Hz  (fCLK / 2^18)
//   10 = AC at 31.25 Hz   (fCLK / 2^16)
//   11 = AC at fDR/4      (62.5 Hz at 250 SPS)
...
struct ImpedanceConfig {
    ...
    // Use AC excitation at fDR/4 (62.5 Hz at 250 SPS). FLEAD_OFF=0b11.
    bool ac_mode = true;
    ...
    [[nodiscard]] constexpr uint8_t loff_register() const {
        ...
        if (ac_mode) val |= 0x03;                            // bits [1:0] = 11 → AC at fDR/4
        // else bits [1:0] = 00 → DC lead-off
        return val;
    }
